...
#############################################REPORT FACTURAS #####################################################
    ...
    def reportFacturas():
        try:
            ...
            var.report.setFont('Helvetica-Bold', size=10)
            var.report.drawString(50, 675, items[0])
            var.report.drawString(100, 675, items[1])
            var.report.drawString(150, 675, items[2])
            var.report.drawString(250, 675, items[3])
            var.report.drawString(300, 675, items[4])
            var.report.drawString(350, 675, items[5])
            var.report.line(50, 670, 570, 670)
            print(252)
            # Database query
            query = QtSql.QSqlQuery()
            ...
            if query.exec():
                ...
                while query.next():
                    if j <= 80:
                        var.report.drawString(450, 90, 'Pagina Siguiente')
                        var.report.showPage()  # Create a new page
                    ...
                    var.report.setFont('Helvetica', size=9)
                    var.report.drawString(50, j, str(query.value(0)))
                    var.report.drawString(100, j, str(query.value(1)))
                    var.report.drawString(150, j, str(query.value(2)))
                    var.report.drawString(250, j, str(query.value(3)))
                    var.report.drawString(300, j, str(query.value(4)))
                    var.report.drawString(350, j, str(query.value(5)))
                    j -= 25
            else:
    ...
    def topInformeFactura(titulo):
        try:
            ...
            if isinstance(logo, Image.Image):
                var.report.line(50, 800, 525, 800)
                var.report.setFont('Helvetica-Bold', size=14)
                var.report.drawString(55, 785, 'Games Teis')
                ...
                var.report.drawString(290, 785,
                                      f'NÚMERO FACTURA: {var.ui.leCodigoFactura.text()}        Fecha: {fecha}')
